...
const DiagramContainer = styled.div<{ $fullHeight?: boolean }>`
  margin: ${props => props.$fullHeight ? '0' : '16px 0'};
  padding: ${props => props.$fullHeight ? '0' : '20px'};
  background: linear-gradient(135deg, rgba(30, 27, 75, 0.8) 0%, rgba(49, 46, 129, 0.6) 100%);
  border-radius: ${props => props.$fullHeight ? '0' : '12px'};
  border: ${props => props.$fullHeight ? 'none' : '1px solid rgba(139, 92, 246, 0.3)'};
  overflow: hidden;
  display: flex;
  ...
  svg {
    max-width: ${props => props.$fullHeight ? 'none' : '100%'};
    height: auto;
  }
...
const DiagramScrollArea = styled.div<{ $fullHeight?: boolean }>`
  overflow: auto;
  ${props => props.$fullHeight ? `
    flex: 1;
    display: flex;
...
const DiagramHeader = styled.div<{ $fullHeight?: boolean }>`
  display: flex;
  align-items: center;
...
export const MermaidDiagram: React.FC<MermaidDiagramProps> = ({ chart, fullHeight }) => {
  ...
  return (
    <DiagramContainer ref={containerRef} $fullHeight={fullHeight}>
      <DiagramHeader $fullHeight={fullHeight}>
        <DiagramLabel>
      ...
      <DiagramScrollArea $fullHeight={fullHeight}>
