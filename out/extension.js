...
function activate(context) {
    const commentCode = vscode.commands.registerCommand("ola.commentCode", comments_1.generateComment);
    const reviewCode = vscode.commands.registerCommand("ola.reviewCode", review_1.generateReview);
    const refactorCode = vscode.commands.registerCommand("ola.codeRefactor", refactor_1.generateRefactoredCode);
    const optimizeCode = vscode.commands.registerCommand("ola.codeOptimize", optimize_1.generateOptimizeCode);
    const fixCode = vscode.commands.registerCommand("ola.codeFix", (errorMessage) => {
