...
import { IStreamChunk, IStreamOptions, StreamEventType } from "../interface/agent.interface";
...
export class StreamManager {
    ...
    handleError(error: Error) {
        this.flush();
        const metadata: { timestamp: number, stack?: string, message?: string } = { timestamp: Date.now(), }
        this.logger.error('Stream error occurred', { error, streamId: this.streamId });
