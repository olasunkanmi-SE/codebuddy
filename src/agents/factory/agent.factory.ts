...
import { AnnotationRoot, InMemoryStore, MemorySaver } from "@langchain/langgraph";
...
export class AgentFactory implements IAgentFactory {
    ...
    public async createAgent(config?: ICodeBuddyAgentConfig): Promise<ReactAgent<any, any, AnnotationRoot<any>, any>> {
        if (!this.agent) {
