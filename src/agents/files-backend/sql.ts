...
export class SqlJsBackend implements BackendProtocol {
  ...
  async lsInfo(path = "/"): Promise<FileInfo[]> {
    ...
    const stmt = this.db.prepare(
      "SELECT path, content, modified_at FROM files"
    );
  ...
  async read(filePath: string, offset = 0, limit = 2000): Promise<string> {
    ...
    const stmt = this.db.prepare(
      "SELECT content, created_at, modified_at FROM files WHERE path = :p LIMIT 1"
    );
    stmt.bind({ ":p": p });
  ...
  async grepRaw(
    ...
    const globRegex = globToRegExp(
      patternGlob.startsWith("/") ? patternGlob : `/${patternGlob}`
    );
    const pfx = path ? path.replace(/\/+$/, "") : "/";
  ...
  async write(filePath: string, content: string): Promise<WriteResult> {
    ...
    const sel = this.db.prepare(
      "SELECT created_at FROM files WHERE path = :p LIMIT 1"
    );
    sel.bind({ ":p": p });
  ...
  async edit(
    ...
    const p = this.nsPath(filePath);
    const sel = this.db.prepare(
      "SELECT content, created_at FROM files WHERE path = :p LIMIT 1"
    );
    sel.bind({ ":p": p });
    if (!sel.step()) {
      sel.free();
    ...
    if (replaceAll) {
      ...
      if (idx === -1) {
        ...
        newContent =
          ...
          newString +
          currentContent.slice(idx + oldString.length);
      }
    }
...
export function createSqljsBackendFactory(params: {
  ...
  return function backendFactory(
