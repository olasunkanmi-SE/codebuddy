        ...
        _view.webview.onDidReceiveMessage(async (message) => {
          ...
          switch (message.command) {
            ...
            case "configure-connector": {
              try {
                const connector = ConnectorService.getInstance().getConnector(
                  message.id,
                );
                if (!connector) {
                ...
                // Special handling for GitHub connector to support OAuth
                  ...
                  if (selection === "Sign in with GitHub (OAuth)") {
                    try {
                      ...
                      if (session) {
                        ...
                        break; // Exit successful OAuth flow
                      }
                    } catch (error: any) {
                ...
                // Configuration Schemas for other connectors
                ...
                if (schema) {
                  ...
                  break; // Exit after guided flow
                }

                // Fallback to Raw JSON Configuration
