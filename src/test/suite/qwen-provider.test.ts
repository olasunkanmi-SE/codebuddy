...
suite('QwenWebViewProvider Test Suite', () => {
  ...
  test('generateResponse should call OpenAI client and return response', async () => {
    ...
    // Check if chat history was updated
    const history = Memory.get(COMMON.QWEN_CHAT_HISTORY);
    // Note: generateResponse updates local chatHistory but might not save to Memory immediately 
    // depending on implementation details of updateProviderChatHistory vs Memory.set
    // But it reads FROM Memory.
  });
});
