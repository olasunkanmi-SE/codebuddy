  ...
  const diagnosticCommand = vscode.commands.registerCommand(
    ...
    async () => {
      try {
        ...
        let chromaStatus = "✅ ChromaDB installed";

        // Check default embedding function
        let embedStatus = "❌ Default embedding not available";
        try {
          await import("@chroma-core/default-embed");
          embedStatus = "✅ Default embedding function available";
        } catch (e) {
          embedStatus = "❌ @chroma-core/default-embed not installed";
        }
...
• **Embedding Function**: ${embedStatus}
